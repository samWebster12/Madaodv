...
{
  static TypeId tid = TypeId ("ns3::madaodv::TypeHeader")
    .SetParent<Header> ()
...
// RREQ
//-----------------------------------------------------------------------------
RreqHeader::RreqHeader (uint8_t flags, uint8_t reserved, uint8_t hopCount, uint32_t requestID, Mac48Address dst,
                        uint32_t dstSeqNo, Mac48Address origin, uint32_t originSeqNo)
  : m_flags (flags),
    m_reserved (reserved),
    m_hopCount (hopCount),
...
{
  static TypeId tid = TypeId ("ns3::madaodv::RreqHeader")
    .SetParent<Header> ()
    .SetGroupName ("Madaodv")
...
{
  return 27; //field lengths (28 bytes) - type length (1 byte)
}
...
{
  os << "RREQ ID " << m_requestID << " destination: mac48 " << m_dst
     << " sequence number " << m_dstSeqNo << " source: mac48 "
     << m_origin << " sequence number " << m_originSeqNo
     << " flags:" << " Gratuitous RREP " << (*this).GetGratuitousRrep ()
     << " Destination only " << (*this).GetDestinationOnly ()
     << " Unknown sequence number " << (*this).GetUnknownSeqno ()
     << " Access Point Query " << (*this).GetAccessPointQuery ();
}
...
void 
RreqHeader::SetAccessPointQuery (bool f)
{
  if (f)
    {
      m_flags |= (1 << 2);
...
bool
RreqHeader::GetAccessPointQuery () const
{
  return (m_flags & (1 << 2));
}
...
RrepHeader::RrepHeader (uint8_t reserved, uint8_t hopCount, Mac48Address dst,
                        uint32_t dstSeqNo, Mac48Address origin, Time lifeTime)
  : m_flags (0),
    m_reserved (reserved),
    m_hopCount (hopCount),
    m_dst (dst),
...
{
  static TypeId tid = TypeId ("ns3::madaodv::RrepHeader")
    .SetParent<Header> ()
    .SetGroupName ("Madaodv")
...
{
  return 23; //Same as RREQ - RREQ ID
}
...
{
  os << "destination: mac48 " << m_dst << " sequence number " << m_dstSeqNo;
  os << " source mac48 " << m_origin << " lifetime " << m_lifeTime
     << " acknowledgment required flag " << (*this).GetAckRequired ()
     << " Access Point " << (*this).GetAccessPoint();
}
...
void
RrepHeader::SetAccessPoint (bool f)
{
  if (f)
    {
      m_flags |= (1 << 7);
...
bool
RrepHeader::GetAccessPoint () const
{
  return (m_flags & (1 << 7));
}
...
void
RrepHeader::SetHello (Mac48Address origin, uint32_t srcSeqNo, Time lifetime)
{
  m_flags = 0;
  m_reserved = 0;
  m_hopCount = 0;
...
{
  static TypeId tid = TypeId ("ns3::madaodv::RrepAckHeader")
    .SetParent<Header> ()
    .SetGroupName ("Madaodv")
...
{
  static TypeId tid = TypeId ("ns3::madaodv::RerrHeader")
    .SetParent<Header> ()
    .SetGroupName ("Madaodv")
...
{
  return (3 + 10 * GetDestCount ()); //10 = 6 (MAC Address) + 4 (Seqno)
}
...
RerrHeader::Serialize (Buffer::Iterator i ) const
{
  ...
  std::map<Mac48Address, uint32_t>::const_iterator j;
...
{
  os << "Unreachable destination (mac address, seq. number):";
  std::map<Mac48Address, uint32_t>::const_iterator j;
  for (j = m_unreachableDstSeqNo.begin (); j != m_unreachableDstSeqNo.end (); ++j)
    {
      os << (*j).first << ", " << (*j).second;
...
bool
RerrHeader::AddUnDestination (Mac48Address dst, uint32_t seqNo )
{
...
bool
RerrHeader::RemoveUnDestination (std::pair<Mac48Address, uint32_t> & un )
{
  if (m_unreachableDstSeqNo.empty ())
    {
      return false;
    }
  std::map<Mac48Address, uint32_t>::iterator i = m_unreachableDstSeqNo.begin ();
  un = *i;
...
RerrHeader::operator== (RerrHeader const & o ) const
{
  ...
  std::map<Mac48Address, uint32_t>::const_iterator j = m_unreachableDstSeqNo.begin ();
  std::map<Mac48Address, uint32_t>::const_iterator k = o.m_unreachableDstSeqNo.begin ();
