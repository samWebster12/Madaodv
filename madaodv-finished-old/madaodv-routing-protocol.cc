                             ...
                             MulticastForwardCallback mcb, LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  if (IsMyOwnAddress (origin))
    {
     // std::cout << "is my own packet" << std::endl;
      return true;
    }
  ...
  if (dst.IsMulticast () && dst != Ipv6Address(BROADCAST_ADDR))
    {
  // /    std::cout << "falling inside here" << std::endl;
      NS_LOG_DEBUG ("madaodv does not work with mulitcast address... dropping");
      return false;
    }
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
 ...
 // Mac48Address macAddr = Mac48Address::ConvertFrom(addr);
  ...
  //Make sure interface address is wihtin our specified range
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvAodv, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (BROADCAST_ADDR, MADAODV_PORT));
  socket->SetAllowBroadcast (true);
 ...
 // Ptr<NetDevice> dev = m_ipv6->GetNetDevice (m_ipv6->GetInterfaceForAddress (iface.GetAddress ()));
  //RoutingTableEntry rt (/*device=*/ dev, /*dst=*/ Ipv6Address(BROADCAST_ADDR), /*know seqno=*/ true, /*seqno=*/ 0, /*iface=*/ iface,
  //                                  /*hops=*/ 1, /*next hop=*/ Ipv6Address(BROADCAST_ADDR), /*lifetime=*/ Simulator::GetMaximumSimulationTime ());
  //m_routingTable.AddRoute (rt);
...
RoutingProtocol::NotifyAddAddress (uint32_t i, Ipv6InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses (i) == 2) //mark for change later
    {
      Ptr<NetDevice> dev = l3->GetInterface(i)->GetDevice ();
     // Address addr = dev->GetAddress();
      //Mac48Address macAddr = Mac48Address::ConvertFrom(addr);
    ...
    /*  if (address.GetAddress() != MacToIpv6(macAddr)) // mark get rid of for testing
      ...
      Ipv6InterfaceAddress iface = l3->GetAddress (i, 1); //mark: changed from 0 to 1 <--
...
RoutingProtocol::SendRequest (Ipv6Address dst)
{
  ...
  for (std::map<Ptr<Socket>, Ipv6InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      std::cout << "\n\nTTL sent out is " << (int) ttl << std::endl;
...
{
  NS_LOG_FUNCTION(this);
  std::cout << "sending packet with id " << packet->GetUid() << " to destination " << destination << " through socket " << socket << std::endl;
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv6Address receiver, Ipv6Address src)
{
  ...
  std::cout << "ttl recieved is " << (int) rreqHeader.GetHopCount() << std::endl;
