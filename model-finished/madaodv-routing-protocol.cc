...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  if (iface.GetAddress() != MacToIpv6 (macAddr))
    {
      return;
    }
  ...
  if (l3->GetNAddresses (i) == 1)
    {
      ...
      if (address.GetAddress() != MacToIpv6(macAddr))
      {
        return;
      }
...
RoutingProtocol::RecvAodv (Ptr<Socket> socket)
{
  ...
  NS_LOG_DEBUG ("AODV node " << this << " received a MADAODV packet from " << sender << " to " << receiver);
